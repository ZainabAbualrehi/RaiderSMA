...
public class Model {
    ...
    static ArrayList<Racer> racerList;
    ...
    public Model(String sport) {
    	...
    	else if (sport.contentEquals("racing")) {
 ...
 * @return **/
   ...
   //Tournament Save to File Method
    ...
    //Racer Lookup String
    public static Racer racerLookup(String alias) throws NotFoundException {
    	
    	//Change alias string to all upper case, to eliminate case sensitive entry
        alias = alias.toUpperCase();
        
        //Return wrestler if alias matches racer first name, last name, or user name
        for (Racer racers : racerList) {
            if (racers.getLastName().contains(alias)) {
                return racers;
            } else if (racers.getFirstName().contains(alias)) {
                return racers;
            } else if (racers.getLastName().contains(alias)) {
                return racers;
            }
        }
        
        //Throw an exception if the requested alias racer is not found
        throw new NotFoundException(alias);
    }
 ...
 public static void importRacerFromFile(String filePath) {
        ...
        while (s.hasNextLine()) {
            try {
                ...
                for (int i = 0; stringLine.hasNext(); i++) {
                	
                    while (stringLine.hasNext()) {
                        token += stringLine.next();
                        if (token.charAt(token.length() - 1) == ',') {
                            tokens.add(token.substring(0, token.length() - 1));
                            token = "";
                            break;
                ...
                if (tokens.size() == 2) {
                    r = new Racer(tokens.get(0), tokens.get(1));
                } 
        ...
        System.out.println("Successfully imported " + importCount + " Racers.");
    }
    ...
    public static ArrayList<Racer> printRacers() {
        System.out.println("List of Racers: ");
        for (int i = 0; i != racerList.size(); i++) {
            System.out.println(racerList.get(i));
        }
        return racerList;
    }
